...
#include <stdlib.h>
    ...
    const int n=21;
    int st[n][n]; //матрица
    int i,j; //станд. счетчики
    int m=1, r=16; //переменные помогающие в движении пакмена
    int way; //переменная, отвечающая за направление пути пакмена
    int ahead1=3, back1=0, t1=6, k1=17; //переменные для 1 призрака
    int ahead2=8, back2=0, t2=2, k2=11; //переменные для 2 призрака
    int ahead3=3, back3=0, t3=2, k3=5; //переменные для 3 призрака
    int ahead4=5, back4=0, t4=16, k4=10; //переменные для 4 призрака
    int ahead5=3, back5=0, left5=0, right5=0, t5=16, k5=4; //переменные для 4 призрака
    int money=0; //счетчик монет

//Построение лабиринта
void labirint (void)
{
    int i,j;
    for (j=1; j<=n; j++) st[1][j]=22; /*Заполнение лабиринта*/
...
{
    scanf("%d", &way);
    switch(way){ /*Выбор пути для пакмена*/
    case 2: 
        {
        printf("Up.\n");
